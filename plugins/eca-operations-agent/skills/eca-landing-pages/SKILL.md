...
description: "Build a high-converting landing page for an ad campaign directly into a Shopify theme — choose a format from Hero Product, Listicle, Social Proof, Kit or Subscription Bundle, and Advertorial, match the copy to the actual ad angle, and install it as editable theme sections. Use whenever the member wants a landing page, ad landing page, campaign page, LP, \"a page for this ad\", a sales page, an advertorial page, a listicle page, a bundle or kit page, wants to fix an ad-to-page mismatch, improve post-click conversion, or asks to build or install a page into their Shopify theme. Runs as an interactive guide where the member leads, then writes a brief they confirm before anything is built. Requires the Shopify CLI: it duplicates the live theme into a dev theme, never touching live, installs the section library and page template, and creates the page unpublished for review. Works with any Online Store 2.0 theme."
---
...
- **Offer layout: three options, and the image can be switched off.** *Image left* (default), *image right*, or **image/video as background** with the copy sitting on top. The background layout is the one to reach for when the member has a strong lifestyle shot or a short looping video — it makes the final ask feel like a brand moment rather than a product row. It ships with an adjustable dark overlay, a soft panel behind the text and centred copy, all of which exist for one reason: **legibility**. Always look at the rendered page and raise the overlay if the headline is fighting the photo — text you can't read costs more than a nice picture gains. **Use a background video only if it's short, silent and loops cleanly**; it autoplays muted, and a long or heavy file will slow the page down at exactly the wrong moment. **Switch the image off entirely** (*Show the image / video*) when the page has already shown the product several times above — by the offer section the job is the ask, and a fourth product shot can dilute it. A text-only offer block centres itself automatically.
...
- **The in-the-box section is block-based (`eca-lp-in-the-box-v2`).** Heading, intro text, each kit item, the total-value line, the button and the guarantee are all **movable blocks**, so the member can reorder them or drop any of them in the theme editor. The image column is section-level: position it left or right, or switch it off for a text-only list. **Seed only the blocks the offer needs** — a kit with no bonus items doesn't need a value line, and a page that already has the offer section below doesn't need a second button here. **Never invent a struck-through "total value"** — only use a figure the member can genuinely stand behind, since inflated RRP comparisons are a consumer-law risk in most markets. Ask if it isn't in `brand-data.md`.
...
- **Run the colour-scheme adapter before every push.** `scripts/adapt-color-schemes.py <theme-dir>` — on themes without colour schemes it strips the setting so Shopify doesn't show a red "color schemes must be defined" banner across the member's editor. See `references/shopify-build.md` §3c. Never leave that warning on a member's screen; it reads like the page is broken when it isn't.
...
- **Benefit icons and images are sized at section level.** One set of controls — size, position (above or beside the text), cropping, corner rounding and text alignment — applies to every item, so the grid stays consistent instead of becoming a jumble of mismatched images. **Photos want 2 columns and a larger size; line icons want 3-4 columns and stay small.** For a circular avatar-style crop, set corner rounding to half the size and cropping to "Fill the space".
...
- **Advertorials open with the editorial masthead (`eca-lp-editorial-head`), not the hero.** An advertorial only works if the top of the page reads like an article: ad disclosure, optional category label, an **editorial headline** (a claim or a story, not a product name), a standfirst that makes them keep reading, a rule, then a **byline with the author's photo and a date**, then the image or video. That masthead sits in a narrow column (~720px) because line length is most of what makes a page *feel* like journalism — a full-width headline reads as marketing no matter how it's written.
- **The disclosure block is not optional, and neither is honesty about who wrote it.** Presenting advertising as editorial without a clear label is regulated in most markets (ACCC in Australia, FTC in the US, ASA in the UK) and platforms reject it too. Keep the disclosure block, use a plain word ("Advertisement", "Sponsored", "Paid partnership"), and leave it at the top where it's seen before the headline. **Never invent an author, a journalist, a publication or a masthead** — the byline must be a real person at the brand who wrote or approved the piece, and the author photo must be that person. If the member has no one to put their name to it, say so and use the founder. Fabricating a reporter is the single fastest way to turn a good advertorial into a legal problem.
- **Don't run the disclosure and byline twice.** The article-body section has its own disclosure and byline fields from before the masthead existed — leave them blank when `eca-lp-editorial-head` is on the page. One source of truth per page.
- **Use a real date.** The byline can show today's date automatically, which suits an evergreen page, but never pair an auto-date with time-specific claims ("this week only", "just released"). If the piece refers to a moment, hard-code the real date.
