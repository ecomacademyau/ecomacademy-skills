...
| **Full page inventory (every URL)** | Every published product/collection/page/blog is accounted for, with its status (indexed / not-indexed & why / canonical / duplicate / thin) | Pull all URLs from Shopify + `/sitemap.xml`, cross-check vs **GSC index coverage**, and run a **DataForSEO On-Page crawl** for a page-by-page status |
| **Duplicate / thin body copy (text)** | Product, collection & page descriptions are **unique** — no identical or near-identical copy reused across products/collections, no manufacturer boilerplate, no sale-clone carrying the main product's text; each key page has enough useful copy | **Pull every product/collection/page description + meta from Shopify and compare them for similarity** (identical & near-duplicate); supplement with DataForSEO Content Analysis. Rewrite duplicates or consolidate the pages |
| **Duplicate content (Shopify traps)** | No duplicate URLs competing: `/products/` vs `/collections/…/products/` canonicalised to the main product; variant & **pagination** URLs canonical; filtered/sorted/tag URLs noindexed or blocked; no near-duplicate/sale/test product copies; no duplicate titles/metas across pages | Fix canonicals; noindex/AJAX filters; consolidate or 301 duplicate & sale-clone products; unique titles/metas |
